...
точку контроля — одобрение готового поста перед публикацией.
...
- **Единственная ручная точка.** Статус `approved` ставит только человек
  после одобрения HTML-превью, эмулирующего вид поста в Telegram. Всё
...
4. Оркестратор ставит `status: approved`, кладёт JSON и картинку в
   очередь на сервере.
5. Сервер по будням в 7:30 МСК публикует самый ранний одобренный пост.
   Автор ничего не делает.
