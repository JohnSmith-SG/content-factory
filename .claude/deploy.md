...
**Важно при каждом редеплое `bot_listener.py` на сервер:** в репозитории
`CHANNEL`/`DISCUSSION_GROUP_ID` — заглушки (реальные значения принципиально
не коммитятся, см. шапку этого файла). После `vps.sh put server/bot_listener.py
...` обязательно вручную подставить в серверный файл настоящий
`DISCUSSION_GROUP_ID` (значение — в `.claude/deploy.local.md`, либо свежее —
через `getChat` на `@ai_pro_cg`, поле `linked_chat_id`) и перезапустить
`cf-bot-listener.service`.

**Прецедент (обнаружено 2026-09-13):** при одном из более ранних редеплоев
это не сделали, сервер остался с заглушкой `-100_0000000000` вместо
реального ID. Из-за этого `handle_message` переставал распознавать
сообщения из группы-обсуждения как комментарии — они тихо проходили мимо
и модерации, и логирования, без единой строки в журнале сервиса. Так
пропал (безвозвратно — Telegram уже отдал апдейт, offset продвинулся)
комментарий владельца канала под постом от 2026-09-10, обнаружен только
потому что пользователь помнил, что писал его лично. Заодно это означало,
что модерация неподписчиков тоже не работала всё это время. Чинить —
сверкой `linked_chat_id` через `getChat`, не только по этому файлу (сам
файл мог устареть, если группу пересоздавали).

Заодно в этом же прецеденте нашли и починили отдельный баг в
`moderate_comment()`: комментарий «от лица канала» (Telegram-опция для
владельца, отдельная от «остаться анонимным» в самой группе) приходит с
`sender_chat` = канал и `from` = служебный псевдо-бот-аккаунт — старый код
отбрасывал такие сообщения веткой «is_bot → просто ещё один бот». Теперь
`sender_chat.type == "channel"` проверяется отдельно, до проверки `is_bot`,
и логируется как легитимный комментарий.
